  ...
  <div class="restores-page">
    ...
    <el-card>
      <div class="toolbar-row">
        <el-select v-model="phaseFilter" clearable style="width: 220px" :placeholder="t('restores.restorePhaseFilter')">
          <el-option :label="t('restores.restorePhaseAll')" value="all" />
...
const fetchRestores = async () => {
  ...
  try {
    const options = {}
    if (phaseFilter.value && phaseFilter.value !== 'all') {
      options.phase = phaseFilter.value
    }
    const res = await getRestores(undefined, options)
