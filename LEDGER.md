...
| ADR-047 | **禁止幽灵配置（No Phantom Config）**：配置值必须有断了会大声失败的消费链路（跨层绑定有端到端测试 / 传子系统的 key 对照真实 schema / 每个 key 指得出消费者行）。机械执行=CI `Phantom Config Guard` + Helm 接缝测试 + `internal/velerons`/`supkube.veleroNamespace` 单一真源。教训来源=2026-06-04 Velero ns 事故（`namespaceOverride` 死值 + 后端 80+ 硬编码 ns）。配套 ENGINEERING.md Rule I + 座右铭 | Mars 决策 / Claude 起草 / 2026-06-04 | **草稿** ✅（ENGINEERING.md Rule I 已写 + CI/脚本已落；架构设计.md §9 正文待补）| ENGINEERING.md Rule I / 架构设计.md §9（待写）/ hack/check-phantom-config.sh |
