...
export default {
  ...
  // v0.8.2 Transform Sets
  transformSets: {
    title: 'Transform Sets',
    desc: 'Transform Sets are bundles of JSONPath patches Velero applies during restore. They fix structural conflicts (NodePort, clusterIP, Ingress host, image registry, …) when restoring across namespaces or clusters. Built-in templates cover the most common cases; you can clone and customize them.',
    create: 'Create Transform Set',
  ...
  restoreDrawer: {
    ...
    fixApplied: 'Resolved by {name}',
    fixCreatedToast: 'Transform Set "{name}" created and selected for this restore',
    // v0.7.12 Pre-flight
  ...
  policies: {
    ...
    // with code references; only the user-visible values change.
    actionSnapshot: 'L1 Local',
    actionSnapshotExport: 'L2 Local + Cloud',
    frequency: 'Frequency',
    onDemand: 'On Demand',
    ...
    dataPath: 'Data Path',
    dataPathHelp: 'How volume data is protected. The two columns mirror the L1/L2 Action above.',
    dataPathSection: {
      snapshotOnly: 'Local Backup only',
      snapshotOnlySub: 'L1 — fast recovery from in-cluster MinIO',
      snapshotExport: 'Local + Cloud Backup',
      snapshotExportSub: 'L2 — local fast recovery + off-site DR'
    },
    dataPathChoice: {
      csi: 'CSI Snapshot',
      mover: 'Data Mover',
    ...
    alwaysOn: 'Always on',
    enable: 'Enable',
    snapshotOnlyWarn: 'Without the Cloud half, this policy keeps only an in-cluster copy — vulnerable to cluster-wide failures. For DR you need both Local + Cloud.',
