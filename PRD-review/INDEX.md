...
| PRD-009 | Policy 对齐 Kasten + Import Policy | **改正中**（2026-06-02 v2 G1-G5 全闭环, 等 Mars 重审） | Phase 1：E3 已闭环。**v2 (报告⑥)**：**G5 (High)** §8.2 Phase 2 DoD 14 条 + §9 Phase 2 任务 7 阶段 + §9.3 风险评级独立写（不再 0 迁移风险）/ **G1 (Med)** §4.5.3 RPO 卖点诚实化撤回"10x RPO"过承诺 / **G2 (Med)** 头表 backupSyncPeriod 维持默认 60s 防退化 / **G3 (Low)** §4.5.4 warn 模式 hover tooltip 标"可能未完成半成品" + 列表行加来源 backup phase / **G4 (Low)** §4 Action Type pill 加 inline alert 保存后不可改 + Save confirm dialog + 编辑灰化只读。fingerprint 三档是 007 P3 的好落地 | 报告④·三 + 报告⑥·三 |
